...
let selectUl = document.querySelector(".selectClass ul");
...
addClass.addEventListener("click", ()=> {
    ...
    if (classname!=null && classname!="")
    {
        ...
        if (controller)
        {
            ...
            selectUl.innerHTML+=`
            <li id="class-li-item">${classname.toUpperCase()}<i id="remove-class" class="bi bi-x-circle"></i>
            </li>
            `;
...
function getStorageUI()
{
    ...
    if (Class.length>0)
    {
        Class.forEach(class_ => {
            selectUl.innerHTML+=`
            <li id="class-li-item">${class_}<i id="remove-class" class="bi bi-x-circle"></i>
            </li>
            `;
        });
    }
